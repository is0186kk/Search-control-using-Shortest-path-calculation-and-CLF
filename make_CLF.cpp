...
#define map_size_xmax 300.0 //マップサイズ（横）
#define map_size_ymax 300.0 //マップサイズ（縦）
		...
		for (int i = xnum_min; i < xnum_max; i++){
			for (int k = 0; k < obstacle.data[j][i].size(); k++){
				if (point_len(xin, obstacle.data[j][i][k].x, yin, obstacle.data[j][i][k].y) < rrin * rrin){
					if (a == 0)return 1;
					//↑壁と近い（より細かく離散化する必要はない）
					//↓壁と近い（より細かく離散化する必要があるため，細かい離散化関数を呼び出す）
...
double v_min(GRAPH *g, OBSTACLES obstacle,
	...
	for (int p = 0; p < g->node.size(); p++){
		v = g->node[p].v(xin, yin) + dist[p];//ここが重くなる原因であると考えている
		for (int j = ynum_min; j < ynum_max; j++){
			for (int i = xnum_min; i < xnum_max; i++){
				for (int k = 0; k < obstacle.data[j][i].size(); k++){
...
int main() {
	...
	OBSTACLES obstacle("wada_lab.txt", xmin, xmax, ymin, ymax, rmax);
