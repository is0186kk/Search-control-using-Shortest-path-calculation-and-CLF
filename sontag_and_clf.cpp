...
const double DT = 1e-5;                 //オイラー法の時間パラメータ
...
const double r_next_margin = 1.01;      //半径更新時のマージン．>1であれば良い．
const double r_first_level = 12.0;       //第1レベルの離散化における円の半径.
//r_first_level > sqrt(2.0)/2.0 * discretization_widthを満たす必要あり
const int level_max = 4;                //離散化の再起プロセスを何回くりかえすか

const double multi_layer_minimum_method_c = 10.0;//多層最小射影法における底上げ値．
//------------------------------------------------------------------------------------------------------
...
public:
	...
	vector< vector< vector<xypoint> > > data9;//三次元配列に障害物の位置情報を保存（９近傍にあるものを保持）

	//初期化
	OBSTACLES(const char *str);
	//与えられた（ｘ，ｙ）座標が属する区画の区画座標を返す関数
	//const修飾子を追加していて，ざっくり言えば，「内部変数をいじるな！」と指定してある．
	void calc_grid(double xin, double yin, int *xgrid, int *ygrid) const;
	//特定のグリッドにある点を全て列挙する
	...
	while (!ifs.eof()){
		...
		for (int yi = y - 1; yi <= y + 1; yi++){
			for (int xi = x - 1; xi <= x + 1; xi++){
				if (0 <= yi && yi < ynum && 0 <= xi && xi < xnum){
					xypoint inpoint = { xin, yin };
					data9[yi][xi].push_back(inpoint);
				}
			}
		}
	}
...
//与えられた（ｘ，ｙ）座標が属する区画の区画座標を返す関数--------------------------
void OBSTACLES::calc_grid(double xin, double yin, int *xgrid, int *ygrid) const{
	*xgrid = (int)((xin - xmin) / lwidth);
	*ygrid = (int)((yin - ymin) / lwidth);
}
//特定のグリッドにある点を全て列挙する
...
public:
	...
	double lwidth;
	int xnum, ynum;
	vector< vector< vector<int> > > data;//三次元配列にノードのインデックスを保存 [gridx][gridy][number_of_node]=nodeindex
	vector< vector< vector<int> > > data9;//グリッドの九近傍のデータを保存しておく

	//初期化
	GRAPH(OBSTACLES &obstacle);
	//与えられた（ｘ，ｙ）座標が属する区画の区画座標を返す関数
	void calc_grid(double xin, double yin, int *xgrid, int *ygrid);
	//ノード追加
...
//与えられた（ｘ，ｙ）座標が属する区画の区画座標を返す関数--------------------------
void GRAPH::calc_grid(double xin, double yin, int *xgrid, int *ygrid){
	*xgrid = (int)((xin - xmin) / lwidth);
	*ygrid = (int)((yin - ymin) / lwidth);
}
//ノード追加--------------------------------------------------------------------
void GRAPH::add_node(NODE n){
	...
	for (int yi = y - 1; yi <= y + 1; yi++){
		for (int xi = x - 1; xi <= x + 1; xi++){
			if (0 <= yi && yi < ynum && 0 <= xi && xi < xnum){
				data9[yi][xi].push_back(current_index);
			}
		}
	}
}
...
//グラフの情報表示------------------------------------------------------------
void GRAPH::print_abstract(void){
	cout << "nodenum : " << node.size() << endl;
	int edgenum = 0;
	for (int i = 0; i < edge.size(); i++)edgenum += edge[i].size();
...
//------------------------------------------------------------------------------------------------------
//edge生成--------------------------------------------------------------------
void make_edge(GRAPH *g, const OBSTACLES &obstacle){
	//エッジを保存するための領域確保
	...
	for (int j = 0; j < g_node_size; j++){//From i To jにエッジがはれるか
		...
		//九近傍グリッド内に中心を持つノードを全探索
		int xgrid, ygrid;
		g->calc_grid(xin, yin, &xgrid, &ygrid);
		int indmax = g->data9[ygrid][xgrid].size();
		for (int m = 0; m < indmax; m++){
			//九近傍内にあるノードの番号をiに代入
			int i = g->data9[ygrid][xgrid][m];
			...
			if (dx * dx + dy * dy < nr){//ノードjの円にノードiの中心があるなら
				...
				double edge_cost = v + multi_layer_minimum_method_c;// / node_cost;
...
int check_wall(const OBSTACLES &obstacle, double xin, double  yin, double rin){
	//九近傍グリッド内に中心を持つ障害物を全探索
	int xgrid, ygrid;
	obstacle.calc_grid(xin, yin, &xgrid, &ygrid);
	int indmax = obstacle.data9[ygrid][xgrid].size();
	for (int m = 0; m < indmax; m++){
		double obx = obstacle.data9[ygrid][xgrid][m].x;
		double oby = obstacle.data9[ygrid][xgrid][m].y;
...
void make_node_sub(GRAPH *g, const OBSTACLES &obstacle, double xin, double yin, double r, int level){
	...
	if (iswall == 0){
		...
		if (level < level_max){
			double r_next = r / sqrt(2.0) * r_next_margin;//r_next_marginはマージン
			double d = r / 2.0;
...
double v_min(GRAPH *g, double xin, double yin, vector<double> &dist, int *hoji_i){
	...
	for (int m = 0; m < indmax; m++){
		...
		int i = g->data9[ygrid][xgrid][m];
		//ノード番号pに対して演算
		double ox = g->node[i].x;
		double oy = g->node[i].y;
		double obr = g->node[i].r;
		if (point_len(xin, ox, yin, oy) >= obr * obr){
			v = INF;
		}
		else{
			v = g->node[i].v(xin, yin) + dist[i];
		}
		if (v < min){
			min = v;
			*hoji_i = i;
...
void print_graph(GRAPH *g, const OBSTACLES &obstacle, vector<double> &dist){
	...
	double vmax = 60000;
	double meshwidth_x = (xmax - xmin) / 300.0;
	double meshwidth_y = (ymax - ymin) / 300.0;

	//それっぽいvmaxを調べる＝全ノード中心におけるＶの値の最大値ｘ１．１倍にしてみる
	vmax = 0;
	for (int i = 0; i < g->node.size(); i++){
		int dummy;
		double v = v_min(g, g->node[i].x, g->node[i].y, dist, &dummy);
		if (v != INF){
			v *= 1.1;
			if (vmax < v)vmax = v;
		}
...
int main() {
	...
	OBSTACLES obstacle(obstacle_file);//障害物構造
	GRAPH g(obstacle);//グラフ構造
	...
	g.print_abstract();                       //作成されたグラフの概要表示（ノード数，エッジ数）
