...
void dissasembled_differential(GRAPH *g, OBSTACLES obstacle, double x[2], double p[2], vector<double> &dist){
	int hoji_i[1] = { 0 };
	int i;
	v_min(g, obstacle, x[0], x[1], dist, hoji_i);
	i = hoji_i[0];
...
//オイラー法---------------------------------------------------------------------
void calc_euler(GRAPH *g, OBSTACLES obstacle, double start[2], vector<double> &dist, int s){
	double u[2] = { 0.0, 0.0 };
	...
	for (time = 0.0; time <= 200.0
		&& abs(x[0] - g->node[s].x) > 0.000001
		&& abs(x[1] - g->node[s].y) > 0.000001; time += DT){
...
int main() {
	...
	calc_euler(&g, obstacle, start, dist, s);//移動経路計算
	return 0;
}
